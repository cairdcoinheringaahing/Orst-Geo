...
def abs_inequality(x, y, mode = 'eq'):
    x = abs(x)
    if mode == 'eq':
        return x == y
    if mode == 'ge':
...
def next_prev(*args, func = None, mode = None):
    if mode is None or func is None:
        return

    add = 1 if mode == 'next' else (-1 if mode == 'prev' else TypeError)

    if len(args) == 1:
        arg = args[0]
        
        if func == 'prime':
            index = prime_index(arg)
            index += add
            return nfind(isprime, index)[-1]

        if func == 'fib':
            infseq = InfSeq['F']
            index = 0
            while arg >= next(infseq):
                index += 1
            index += add
            infseq.drop()
            return infseq.take(index)[-1]
...
def prime_index(value):
    primes_list = nfind(isprime, value)
    return primes_list.index(value) + 1
